...
public interface MuserRepoPageable extends PagingAndSortingRepository<Muser, Long> {
	 @Query("SELECT new com.knowledgeVista.User.MuserDto(u.userId, u.username, u.email, u.phone, u.isActive, u.dob, u.skills,u.institutionName) " +
	    ...
	    @Query("SELECT new com.knowledgeVista.User.MuserDto(u.userId, u.username, u.email, u.phone, u.isActive, u.dob, u.skills,u.institutionName) " +
	           "FROM Muser u WHERE UPPER(u.role.roleName) =UPPER(:rolename) AND u.institutionName = :institutionname")
	    ...
	    @Query("""
	    	       SELECT new com.knowledgeVista.User.MuserDto(
	    	               u.userId, u.username, u.email, u.phone,
	    	               u.isActive, u.dob, u.skills, u.institutionName)
	    	       FROM Muser u
	    	       WHERE UPPER(u.role.roleName) = UPPER(:rolename)
	    	         AND UPPER(u.email)         = UPPER(:email)
	    	      """)
	    	Optional<MuserDto> findByRoleNameAndEmail(@Param("rolename") String roleName,
...
@Query("SELECT new com.knowledgeVista.User.MuserDto(m) FROM Muser m WHERE " +
	       "(:username IS NULL OR LOWER(m.username) LIKE LOWER(CONCAT('%', :username, '%'))) AND " +
	       "(:email IS NULL OR LOWER(m.email) LIKE LOWER(CONCAT('%', :email, '%'))) AND " +
	       "(:phone IS NULL OR m.phone LIKE %:phone%) AND " +
	       "(:dob IS NULL OR m.dob = :dob) AND " +
	       "(:skills IS NULL OR LOWER(m.skills) LIKE LOWER(CONCAT('%', :skills, '%'))) AND " +
	       "m.role.roleName = :roleName AND m.institutionName = :institutionName")
	Page<MuserDto> searchByRoleAndFilters(
	    @Param("username") String username,
	    @Param("email") String email,
	    @Param("phone") String phone,
	    @Param("dob") LocalDate dob,
...
@Query("SELECT new com.knowledgeVista.User.MuserDto(u.userId, u.username, u.email, u.phone, u.isActive, u.dob, u.skills, u.institutionName) " +
	       "FROM Muser u WHERE u.role.roleId = :roleId AND u.institutionName = :institutionName")
	org.springframework.data.domain.Page<MuserDto> findByRoleIdAndInstitutionName(@Param("roleId") Long roleId,
	                                               @Param("institutionName") String institutionName,
	                                               Pageable pageable);
